      ...
      <div className='grid grid-cols-1 md:grid-cols-12 gap-6'>
        ...
        <div className='md:col-span-3 p-5'>
          ...
          <div className='flex flex-col'>
            ...
            <div className='flex flex-col items-center justify-center gap-4 bg-white rounded-2xl p-5 border border-pink-300'> {/* Example background color */}
              ...
              <div className='w-full'>
                <Button onClick={() => { setSelectedDivision(""); setSelectedTourType(""); }} className=' px-6 py-3 w-full'>Reset</Button>
              </div>

            </div>

            {/* division price range */}
            <div className='mt-5 bg-white rounded-2xl py-5 px-2 border border-pink-300'>

              <Form {...form}>

                <form className='flex flex-col items-center justify-center gap-4 px-4' onSubmit={form.handleSubmit(priceRangeHandle)}>
                  <FormField
                  ...
                  <Button onClick={() => form.reset()} type="button" className="flex-initial w-full px-6 mt-1">
                    Reset
                  </Button>
        ...
        {/* right ride container */}
        <div className='md:col-span-9 p-5 rounded-2xl '>
                ...
                {!tourLoading && tours?.map((tour: ITourresponse) => (
                  ...
                  >
                    ...
                    <div className="relative h-56 overflow-hidden">
                      ...
                      <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-full">
                        <div className="flex items-center gap-1">
                          <DollarSign className="w-4 h-4 text-[#EF1A53]" />
                          <span className="font-bold text-[#EF1A53]">{tour.costFrom}</span>
                        </div>
                      </div>

                      {
                        tourType.map((item: { _id: string, name: string }) => item._id === tour.tourType && (
                          <div className="absolute top-4 left-4">
                            <span className="bg-[#EF1A53] text-white px-3 py-1 rounded-full text-sm font-medium">
                              {item.name || "Genarel"}
                            </span>
                          </div>
                        ))
                    ...
                    < div className="p-6" >
                      {/* Title and Location */}
                      <div className="mb-4" >
                        <h3 className="text-2xl font-bold text-gray-900 mb-2 line-clamp-1">{tour.title}</h3>
                        <div className="flex items-center text-gray-600">
                          <MapPin className="w-8 h-8 mr-2 text-[#EF1A53]" />
                      ...
                      <p className="text-gray-600 mb-6 line-clamp-2">
                        {tour.description || "lorem250 ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."}
                      </p>
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-6">
                        <div className="flex items-center space-x-2">
                          <div className="p-2 bg-blue-50 rounded-lg">
                            <Users className="w-7 h-7 text-[#EF1A53]" />
                          </div>
                          <div>
                            <p className="text-xs text-gray-500">Max Guests</p>
                            <p className="font-semibold">{tour.maxGests || "1"} </p>
                          </div>
                        </div>

                        <div className="flex items-center justify-center space-x-2">
                          <div className="p-2 bg-green-50 rounded-lg">
                            <Calendar className="w-7 h-7 text-[#EF1A53]" />
                          </div>
                          <div>
                            <p className="text-xs text-gray-500">Min Age</p>
                            <p className="font-semibold">{tour.minAge || "No Age Limit"}+</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex justify-between items-center pt-4 border-t">
                        <div className="flex items-center">
                          <Star className="w-4 h-4 text-[#FF2056] fill-current" />
                          <Star className="w-4 h-4 text-[#FF2056] fill-current" />
                          <Star className="w-4 h-4 text-[#FF2056] fill-current" />
                          <Star className="w-4 h-4 text-[#FF2056] fill-current" />
                          <Star className="w-4 h-4 text-[#FF2056] fill-current" />
